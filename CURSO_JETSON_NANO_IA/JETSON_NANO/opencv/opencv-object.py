...
camSet='nvarguscamerasrc wbmode=1 !  video/x-raw(memory:NVMM), width=3264, height=2464, format=NV12, framerate=21/1 ! nvvidconv flip-method='+str(flip)+' ! video/x-raw, width='+str(dispW)+', height='+str(dispH)+', format=BGRx ! videoconvert ! video/x-raw, format=BGR ! appsink'
...
while True:
    ...
    cv2.moveWindow('FGmaskComp',0,330)
    ...
    cv2.moveWindow('bg',380,330)

    bg=cv2.cvtColor(bgmask,cv2.COLOR_GRAY2BGR)

    final=cv2.add(fg,bg)
    cv2.imshow('final',final)
    cv2.moveWindow('final',680,0)
